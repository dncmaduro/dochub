...
function value(name: string, input: string | undefined): string {
  ...
  if (!result) throw new Error(`${name} is required when ONLYOFFICE is configured`);
  return result;
}
...
export function loadEditorConfig(
  ...
  if (!/^\d+$/.test(rawTtl))
    throw new Error('ONLYOFFICE_FETCH_TOKEN_TTL_SECONDS must be a positive integer');
  ...
  if (
    ...
    throw new Error('ONLYOFFICE_FETCH_TOKEN_TTL_SECONDS must be between 60 and 3600');
  return {
    publicUrl: httpUrl('ONLYOFFICE_PUBLIC_URL', value('ONLYOFFICE_PUBLIC_URL', env.ONLYOFFICE_PUBLIC_URL)),
    internalApiUrl: httpUrl('ONLYOFFICE_INTERNAL_API_URL', value('ONLYOFFICE_INTERNAL_API_URL', env.ONLYOFFICE_INTERNAL_API_URL)),
    jwtSecret,
    fetchTokenSecret,
